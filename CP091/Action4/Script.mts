...
str_NomCliente   = DataTable("e_Nombre_Cliente",dtLocalSheet)
str_ApeCliente   = DataTable("e_Apellido_Cliente",dtLocalSheet)
str_Tipo_Doc     = DataTable("e_Tipo_Documento",dtLocalSheet)
str_Num_Doc    	 = DataTable("e_Num_Documento",dtLocalSheet)
...
Sub DetallesProducto()
	...
	If JavaWindow("Ejecutivo de interacción").JavaMenu("Acciones").JavaMenu("Pedidos").JavaMenu("Suspensión por Perdida").GetROProperty("enabled") = "1" Then
		JavaWindow("Ejecutivo de interacción").JavaMenu("Acciones").JavaMenu("Pedidos").JavaMenu("Suspensión por Perdida").Select @@ hightlight id_;_5968454_;_script infofile_;_ZIP::ssf4.xml_;_
		else
		DataTable("s_Resultado", dtLocalSheet) = "Fallido"
	    DataTable("s_Detalle", dtLocalSheet) = "No se puede Suspender por Perdida al número: "&DataTable("e_ID_Servicio",dtLocalSheet)&", ya que la opción 'Suspensión por Perdida' esta deshabilitada"
		Reporter.ReportEvent micFail, DataTable("s_Resultado", dtLocalSheet), DataTable("s_Detalle", dtLocalSheet)
...
Sub ActualizarAtributos()
	...
	wait 2
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Actualizar Atributos de").JavaEdit("Nombre").Set str_NomCliente
	wait 2
	...
	wait 2
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Actualizar Atributos de").JavaEdit("Numero de documento presentado").Set str_Num_Doc
	wait 2
	...
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Actualizar Atributos de").JavaButton("Siguiente >").Click
		...
		t = 0
		While ((JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de la orden (Orden").Exist) Or (JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").Exist)) = False
			Wait 1	
			t = t + 1
			If (t >= 180) Then
				DataTable("s_Resultado", dtLocalSheet) = "Fallido"
...
Sub ResumenOrden()
	...
	Select Case DataTable("e_Ambiente", "Login")
		Case "UAT8"
